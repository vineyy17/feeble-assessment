...
export const split = (): void => {
  ...
  hCustom.forEach((item) => {
    ...
    // Check if element is already in viewport (for SSR/production timing issues)
    const checkAndAnimate = () => {
      ...
      if (isInViewport) {
        ...
        // Use Intersection Observer for elements not yet in view
        IO(item).then(() => {
          const elem = item.querySelectorAll('.char');
          gsap.to(elem, {
            opacity: 1,
